...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None, video_start=None):
        ...
        if adapter is not None:
            ...
            gate1_expanded = self.gate1.expand(-1, -1, adapter_len, 1).half()  
        
            # Apply task-specific prompt scaling
        ...
        if adapter is not None:            
            scores_adapter = scores[..., :adapter_len].float()  # [bsz, n_local_heads, slen, adapter_len]
            adapter_scores = F.softmax(scores_adapter, dim=-1).type_as(xq)
            
            gate1_scale = gate1_expanded.transpose(1, 2).tanh()  # [1, adapter_len, n_local_heads, 1]
            gate1_scale = gate1_scale.permute(0, 2, 1, 3)  # [1, n_local_heads, adapter_len, 1]
            gate1_scale = gate1_scale.expand(bsz, -1, -1, -1)  # [bsz, n_local_heads, adapter_len, 1]
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args):
        ...
        # Add new parameters for TQCP
        self.num_qtypes = 10  
        self.qtype_embeddings = nn.Embedding(self.num_qtypes, params.dim)
        self.neg_temp = args.neg_temp  
        self.tqcp_weight = args.tqcp_weight




    def compute_tqcp_loss(self, q_features, prompt_emb, qtype, neg_qtypes, type_weight=None, save_embeddings = False):
        q_repr = q_features.mean(dim=1)  # [B, D]
        bsz = q_repr.size(0)
        prompt_emb = prompt_emb.to(q_repr.dtype)
        prompt_emb_expanded = prompt_emb.unsqueeze(0).expand(bsz, -1, -1)  # [B, L, D]
        attn_scores = torch.matmul(prompt_emb_expanded, q_repr.unsqueeze(-1)).squeeze(-1)
        attn_weights = F.softmax(attn_scores, dim=-1)  # [B, L]
        weighted_prompt = (prompt_emb_expanded * attn_weights.unsqueeze(-1)).sum(dim=1)
        
        pos_type_emb  = self.qtype_embeddings(qtype).to(q_repr.dtype)        # [B, D]
        neg_type_embs = self.qtype_embeddings(neg_qtypes).to(q_repr.dtype)   # [B, N, D]
        neg_type_embs_flat = neg_type_embs.reshape(-1, self.params.dim)  # [B*N, D]
        all_type_embs = torch.cat([pos_type_emb, neg_type_embs_flat], dim=0)
        
        
        logits = torch.matmul(weighted_prompt, all_type_embs.t())  # [B, B*(1+N)]
        logits = logits / self.neg_temp
        
        labels = torch.arange(bsz, device=logits.device)  # [0,1,2,...,B-1]
        per_sample_loss = F.cross_entropy(logits, labels, reduction='none')  # [B]
        
        if type_weight is not None:
            per_sample_loss = per_sample_loss * type_weight.to(per_sample_loss.device)
        loss = per_sample_loss.mean()
        
        if save_embeddings:
            embeddings_dict = {
                'weighted_prompt': weighted_prompt.detach().cpu(),
                'pos_type_emb': pos_type_emb.detach().cpu(),
                'neg_type_embs': neg_type_embs.detach().cpu(),
                'all_type_embs': all_type_embs.detach().cpu()
            }
            return loss, embeddings_dict
        return loss


    def forward(self, data, inference=False, save_embeddings = False, current_epoch = None, question_type = None):
        ...
        embedding_dict = None
        tqcp_loss = torch.tensor(0.0).cuda()
        
        # Loss computation
        if not inference and 'qtype' in data:
            qtype = data['qtype'].cuda()
            neg_qtypes = data['neg_qtypes'].cuda()
            type_weight = data.get('type_weight', None)
            if type_weight is not None:
                type_weight = type_weight.cuda()
                
            if save_embeddings:
                tqcp_loss, embedding_dict = self.compute_tqcp_loss(
                    q_features=vqa_h,
                    prompt_emb=self.adapter_query.weight,
                    qtype=qtype,
                    neg_qtypes=neg_qtypes,
                    type_weight=type_weight,
                    save_embeddings=True
                )
                # Add metadata to embedding dict
                if embedding_dict is not None and current_epoch is not None and question_type is not None:
                    embedding_dict['epoch'] = current_epoch
                    embedding_dict['question_type'] = question_type
            else:
                tqcp_loss = self.compute_tqcp_loss(
                    q_features=vqa_h,
                    prompt_emb=self.adapter_query.weight,
                    qtype=qtype,
                    neg_qtypes=neg_qtypes,
        ...
        if inference:
            ...
            losses = {
                'vqa_loss': vqa_loss,
                'vaq_loss': vaq_loss if self.args.vaq else torch.tensor([0]).cuda(),
                'qav_loss': qav_loss if self.args.qav else torch.tensor([0]).cuda(),
                'tqcp_loss': tqcp_loss * self.tqcp_weight
                }
            if save_embeddings and embedding_dict is not None:
                return losses, embedding_dict
            return losses
