...
def get_args_parser():
    ...
    parser.add_argument('--addapter_len', type=int, default=10, metavar='LENGTH', help='the adapter length')
    ...
    parser.add_argument('--vaq', action='store_true', help='vaq loss') #'store_true'
    parser.add_argument('--qav', action='store_true', help='qav loss') #'store_true'
    ...
    parser.add_argument('--negative_sample_size', type=int, default=7,
                        help='Number of negative question types to sample')
    parser.add_argument('--neg_temp', type=float, default=0.1,
                        help='Temperature for negative guidance')
    parser.add_argument('--tqcp_weight', type=float, default=0.1,
                        help='Weight for TQCP loss')
...
def main(args):
    ...
    adapter_indices = [round(i * (model.params.n_layers - 1) / (model.params.adapter_layer - 1)) 
                    for i in range(model.params.adapter_layer)]
    print("Adapter Injection layer:")
    for idx in sorted(set(adapter_indices)):
        print(f"layer: {idx}")
    ...
    for type in qn_types:
        ...
        args.processed_qtypes = processed_type
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_stats = train_one_epoch(model, data_loader_train, optimizer, epoch, loss_scaler, args=args, current_qtype=type)
