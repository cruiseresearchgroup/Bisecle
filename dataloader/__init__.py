...
def load_data(args, tokenizer, split='train', type=None):
    ...
    if hasattr(args, 'processed_qtypes'):
        dataset.processed_qtypes = args.processed_qtypes
...
def batch_collate(batch):
    ...
    neg_qtypes = torch.stack([batch[i]['neg_qtypes'] for i in range(bs)])
    type_weight = torch.tensor([batch[i]['type_weight'] for i in range(bs)])
