...
class BaseDataset(Dataset):
    ...
    def _get_padding_id(self, text_id):
        padding_text_id = torch.full((len(text_id), self.max_seq_len), -1, dtype=torch.int64)
        for i, tid in enumerate(text_id):
            if len(tid) > self.max_seq_len:
                tid = tid[:self.max_seq_len]
                print('max sequence length overflow')
            padding_text_id[i, :len(tid)] = tid.clone().detach().to(torch.int64)
    ...
    def _get_text_token(self, text, answer):
        ...
        effective_feats = min(self.max_feats, qav_padding_text_id.size(1) - qav_prefix_index)
        ...
        if effective_video_feats > 0:
            ...
            if effective_video_feats < self.max_feats:
                pad_length = self.max_feats - effective_video_feats
                padding = torch.full((pad_length,), self.max_seq_len - 1, dtype=qav_video_index.dtype)
                qav_video_index = torch.cat([qav_video_index, padding])
        else:
